...
void StreamServer::initService(int Port, int BufferSize,
    ...
    if (useHeartBeat) {
        tryAgain = 0;
        do {
            int pthread_res = pthread_create(&thHeartBeat, NULL, &StreamServer::HeartBeat, this);
            if (pthread_res == 0) {
...
void StreamServer::AccpetClientsEvent(void* arg) {
    StreamServer *self = (StreamServer*)arg;
...
void* StreamServer::HeartBeat(void* arg) {
    StreamServer *self = (StreamServer*) arg;

    self->HeartBeatEvent(arg);
}

void StreamServer::HeartBeatEvent(void* arg) {
    StreamServer *self = (StreamServer*) arg;
...
int StreamServer::GetTotalConnectedClients() {
